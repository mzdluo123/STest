...
async fn main() {
    let results = tokio::join!(
        ...
        test_download(
            "http://issuepcdn.baidupcs.com/issue/netdisk/yunguanjia/BaiduNetdisk_7.26.0.10.exe"
        )
    );
    let r = [results.0.unwrap().to_speed(), results.1.unwrap().to_speed(), results.2.unwrap().to_speed()];
    //let r = [results.0.unwrap().to_speed()];
...
async fn test_download(url: &str) -> SError<TestResult> {
    ...
    req.headers_mut()
        .append("range", HeaderValue::from_str("bytes=0-200000000")?);
